...
about(){
    int n;
printf("\n\n      'Transportation System'is a online based system that provides the transportation facility to the customer based on their requirements.\n\nThis system provides different types of Cars (Luxury, SUV)  or Buses  for better travelling. \n");
printf("\nPress 0 to go back- ");
scanf("%d",&n);
if(n==0){
    system("CLS");
...
reservation(){
    ...
    printf("\nDo you Registered to this system? (0 for 'NO' / 1 for 'YES')\n");
    scanf("%d",&agree);
    if(agree==1){
    printf("\n\n\t\t\t\t\t\t\tRESERVATION");
...
//////////////////////////////////////////  history
    ...
    if(rent==NULL){
        ...
        exit(1);
            ...
            fscanf(rt_reserv,"%s",&lo.lu_nm);
            de=lo.lu_nm;
            fclose(rt_reserv);

            rent=fopen("Project_File//Rent//.txt","r");
            fscanf(rent,"%s",&lo.lu_rent);
            ef=lo.lu_rent;
            fclose(rent);
